...
PrimeFaces.BabylonConfigurator = {
    ...
    changeComponentsTheme: function(theme, accentMode) {
        componentTheme = this.getColor(theme, accentMode);
        var library = 'primefaces-babylon';
        var linkElement = $('link[href*="theme.css"]');
        var href = linkElement.attr('href');
        var index = href.indexOf(library) + 1;
        var currentTheme = href.substring(index + library.length);
        this.replaceLink(linkElement, href.replace(currentTheme, componentTheme));
    ...
    getColor: function(name, accentMode) {
        return name + (accentMode ? '-accent' : '');
    },
    ...
    changeMenuToRTL: function() {
        ...
        this.updateNano(wrapper.hasClass('layout-rtl'));
    },
