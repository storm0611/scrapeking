...
public class GmailTrigger1DataDao {

	private static final Logger logger = LoggerFactory.getLogger(GmailTrigger1DataDao.class);

	public static void saveTrackGmailMail(Session session, GmailTrigger1Data trackGmailMail) {
		try {
	...
	public static GmailTrigger1Data getLastSubject(Session session) {
		try {
			Query query = session.createQuery("from GmailTrigger1Data order by trigger1Id desc");
			query.setMaxResults(1);
			List<GmailTrigger1Data> trackGmailMails = (List<GmailTrigger1Data>)query.list();
	...
	public static List<GmailTrigger1Data> getFailedEmails(Session session) {
		try {
			Query query = session.createQuery("from GmailTrigger1Data gtd where gtd.subjectId in (select s.subjectId from Subject s where sent1=false)");
			List<GmailTrigger1Data> gmailTrigger1Datas = (List<GmailTrigger1Data>)query.list();
			if(gmailTrigger1Datas.isEmpty())
				return null;
			return gmailTrigger1Datas;

		} catch (Exception ex) {
			logger.error("method getFailedEmails trigger1 error : ", ex);
			return null;
		}
	}
	...
	public static List<String> getCampaignUniqueEmails(Session session) {
		try {
			Query query = session.createQuery("select gtd.sentFrom from GmailTrigger1Data gtd group by gtd.sentFrom");
			...
			logger.error("method getCampaignUniqueEmails trigger1 error : ", ex);
			return null;
		}
	}

	public static long getSent1TrueCountByEmail(Session session, String email) {
		try {
			Query query = session.createQuery("select count(gtd.sentFrom) from GmailTrigger1Data gtd where gtd.sentFrom=:email and gtd.subjectId in (select s.subjectId from Subject s where sent1=true)");
	...
	public static long getSent1FalseCountByEmail(Session session, String email) {
		try {
			Query query = session.createQuery("select count(gtd.sentFrom) from GmailTrigger1Data gtd where gtd.sentFrom=:email and gtd.subjectId in (select s.subjectId from Subject s where sent1=false)");
			query.setParameter("email", email);
